...
class CoalescedPagedForward:
    ...
    def merge(cls, contexts: Sequence[ARDiffusionPagedForwardContext]) -> CoalescedPagedForward:
        ...
        for context in contexts:
            if not getattr(context, "_prepared", False):
        ...
        width = max(int(context.block_table.shape[1]) for context in contexts)
        block_table = torch.cat(
            [
                torch.nn.functional.pad(context.block_table, (0, width - int(context.block_table.shape[1])))
                for context in contexts
            ],
            dim=0,
        )
        ...
        query_start_loc = torch.tensor(
            ...
            dtype=first.query_start_loc.dtype,
            device=first.query_start_loc.device,
        )
